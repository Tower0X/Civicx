...
export const adminSignup = async (
  ...
  try {
    ...
    const existingAccessCode = await AdminModel.findOne({ adminAccessCode });
    if (existingAccessCode) {
      res.status(400).json({ message: "This admin access code is already taken" });
      return;
    }
    ...
    res.status(201).json({ message: "Admin Signed up!" });
  } catch (err: any) {
    ...
    res
      .status(500)
      .json({ message: "Server error while creating admin. Please try again." });
  }
};
