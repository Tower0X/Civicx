          ...
          {issueTypes.map((type, index) => (
            ...
            >
              <div className="relative h-48 overflow-hidden">
                <img
                  src={type.image}
                  alt={t(`issueTypes.items.${type.key}.title`)}
                ...
                <div className="absolute bottom-4 left-4 text-white">
                  <div className="text-sm font-medium">{t(`issueTypes.items.${type.key}.count`, { defaultValue: type.count })}</div>
                </div>
              </div>
              <CardHeader className="pb-2">
                <CardTitle className="text-lg">{t(`issueTypes.items.${type.key}.title`)}</CardTitle>
              </CardHeader>
              <CardContent>
                <p className="text-muted-foreground text-sm leading-relaxed">
                  {t(`issueTypes.items.${type.key}.desc`)}
                </p>
              </CardContent>
            </Card>
          ))}
